...
export function AdminDashboard({ onExit }) {
  ...
  const [activeTab, setActiveTab] = useState('analytics'); // 'analytics' | 'activity' | 'memories' | 'notifications' | 'audit' | 'answers' | 'users'
  ...
  const [notifIsPublished, setNotifIsPublished] = useState(true);
  const [deletingNotifTarget, setDeletingNotifTarget] = useState(null);
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [rData, actData, auditData, analyticsData, mems, notifsRes] = await Promise.all([
        ...
        fetchUserNotifications({ user_id: 'All', includeUnpublished: true }),
      ]);
  ...
  // Notification Submit Handler (Create)
  const handleSaveNotificationSubmit = async (e) => {
    e.preventDefault();
    ...
    try {
      await createNotification({
        ...
        type: notifType,
        target_user_id: notifTargetUser,
        is_published: notifIsPublished,
      });

      setSuccessNotice('Notification sent successfully ✓');
      ...
      setNotifIsPublished(true);
      loadDashboardData();
    } catch (err) {
      console.error('[Admin] Save notification error:', err);
    }
  };

  // Delete Notification Handler
  const handleConfirmDeleteNotification = async () => {
    if (!deletingNotifTarget) return;
    try {
      await deleteNotification(deletingNotifTarget.id);
      setSuccessNotice('Notification deleted ✓');
      setTimeout(() => setSuccessNotice(''), 3000);
      loadDashboardData();
    } catch (err) {
  ...
  // Toggle Notification Visibility Handler
  const handleToggleNotificationVisibility = async (notifId) => {
    try {
      await toggleNotificationVisibility(notifId);
      setSuccessNotice('Notification visibility updated ✓');
      setTimeout(() => setSuccessNotice(''), 3000);
      loadDashboardData();
    } catch (err) {
  ...
  const filteredNotifications = notificationsList.filter((n) => {
    if (selectedUserFilter !== 'All' && n.target_user_id !== 'All' && n.target_user_id !== selectedUserFilter) return false;
    if (!searchQuery.trim()) return true;
    const q = searchQuery.toLowerCase();
    return (
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-pink-700 font-semibold">
                Notifications, Photo & Memory Wall, User 360° Profile & System Management
              </p>
            </div>
          </div>
        ...
        <div className="glass-panel p-4 rounded-2xl bg-white border border-pink-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="relative w-full sm:w-64">
            <input
              ...
              placeholder="Search user, notification, question..."
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-pink-200 pb-3">
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-2xl font-heading font-extrabold text-xs uppercase tracking-wider transition-all flex items-center space-x-1.5 ${
              activeTab === 'notifications' ? 'bg-pink-500 text-white shadow-md' : 'bg-white text-pink-900 hover:bg-pink-100'
            }`}
          >
            <Bell size={15} />
          ...
          <div className="space-y-6">
            <div className="glass-panel p-6 rounded-3xl bg-white border border-pink-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <h3 className="font-heading font-extrabold text-lg text-pink-950">
                  📢 NOTIFICATION MANAGEMENT ({filteredNotifications.length} items)
                </h3>
                <p className="text-xs text-pink-700">Create, send, publish/unpublish, and manage system notifications</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredNotifications.map((notif) => (
                <div key={notif.id} className="p-4 rounded-3xl bg-white border border-pink-200 shadow-sm space-y-2 text-left flex flex-col justify-between">
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs font-bold">
                      <span className="px-2.5 py-0.5 rounded-full bg-pink-100 text-pink-900 uppercase text-[10px]">
                        {notif.type || 'announcement'}
                      </span>
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] uppercase ${
                        notif.is_published ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'
                      }`}>
                        {notif.is_published ? '🟢 Published' : '🔴 Hidden'}
                      </span>
                    </div>

                    <h4 className="font-heading font-extrabold text-base text-pink-950">{notif.title}</h4>
                    <p className="text-xs text-pink-800 font-body leading-relaxed">{notif.message}</p>

                    <div className="flex items-center justify-between text-[11px] font-bold text-pink-600 pt-1">
                      <span>Target: @{notif.target_user_id || 'All'}</span>
                      <span>{notif.date || notif.time || new Date(notif.created_at).toLocaleDateString()}</span>
                    </div>
                  </div>

                  <div className="pt-3 border-t border-pink-100 flex items-center justify-between text-xs font-bold">
                    <button
                      onClick={() => handleToggleNotificationVisibility(notif.id)}
                      className={`px-3 py-1 rounded-xl text-[11px] ${
                        notif.is_published ? 'bg-rose-100 text-rose-800' : 'bg-emerald-100 text-emerald-800'
                      }`}
                    >
                      {notif.is_published ? '🔴 Unpublish' : '🟢 Publish'}
                    </button>

                    <button
                      onClick={() => setDeletingNotifTarget(notif)}
                      className="px-3 py-1 rounded-xl bg-rose-100 text-rose-700 hover:bg-rose-200"
                    >
                      🗑️ Delete
                    </button>
                  </div>
        ...
        {isNotifModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-pink-950/30 backdrop-blur-md text-left">
            <div className="glass-panel p-6 sm:p-8 rounded-3xl max-w-lg w-full bg-white border-2 border-pink-300 shadow-2xl space-y-4">
              <h3 className="font-heading font-extrabold text-xl text-pink-950 flex items-center space-x-2">
                <Bell size={20} className="text-pink-600" />
                <span>📢 Send New Notification</span>
              </h3>

              <form onSubmit={handleSaveNotificationSubmit} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 💌 New Special Message"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="A special notification message for the user..."
                    className="w-full p-3 rounded-2xl bg-pink-50 border border-pink-200 text-pink-950 text-xs font-bold"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-pink-900 uppercase block mb-1">Notification Type</label>
                    <select
                      value={notifType}
                ...
                <div className="flex items-center space-x-2 pt-1">
                  <input
                    type="checkbox"
                    id="notifIsPublished"
                    checked={notifIsPublished}
                    onChange={(e) => setNotifIsPublished(e.target.checked)}
                    className="w-4 h-4 rounded text-pink-600 focus:ring-pink-400"
                  />
                  <label htmlFor="notifIsPublished" className="text-xs font-bold text-pink-900">Publish immediately 🟢</label>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-pink-950/30 backdrop-blur-md text-center">
            <div className="glass-panel p-6 sm:p-8 rounded-3xl max-w-md w-full bg-white border-2 border-pink-300 shadow-2xl space-y-4">
              ...
              <p className="text-xs text-pink-700 font-semibold bg-rose-50 p-3 rounded-2xl border border-rose-100">
                "{deletingNotifTarget.title}" will be deleted.
              </p>
